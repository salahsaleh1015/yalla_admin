...
class AcceptedOrdersListView extends StatelessWidget {
  const AcceptedOrdersListView({super.key});

  @override
  Widget build(BuildContext context) {
    return BlocProvider<FetchAcceptedOrdersCubit>(
      create:
          (context) => FetchAcceptedOrdersCubit(
            FetchAcceptedOrdersUseCase(getIt.get<FetchOrdersRepoImpl>()),
          )..fetchAcceptedOrders(),
      child: BlocBuilder<FetchAcceptedOrdersCubit, FetchAcceptedOrdersStates>(
        builder: (context, state) {
          var cubit = FetchAcceptedOrdersCubit.get(context);
          ...
          } else if (state is FetchAcceptedOrdersErrorState) {
